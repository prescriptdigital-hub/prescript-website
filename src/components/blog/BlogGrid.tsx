    ...
    <section className="py-12 px-4 sm:px-6 lg:px-8 bg-gray-50">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {visible.map(p => (
            <Link key={p.slug} href={"/blog/" + p.slug} className="group bg-white rounded-2xl border border-gray-100 p-6 flex flex-col hover:shadow-md transition-shadow">
              ...
              <div className="flex items-center justify-between pt-4 border-t border-gray-100">
                <span className="font-sans text-xs text-gray-400">{new Date(p.date).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" })}</span>
                <span className="flex items-center gap-1 text-xs font-sans font-medium text-prescript-green group-hover:underline">Read <ArrowRight size={12} /></span>
              </div>
            </Link>
          ))}
        </div>
